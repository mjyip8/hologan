...
class HoloGAN:
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(data_loader):
          if (data[0].shape[0] == self.batch_size):
            ...
            if i == np.ceil(float(len(img_list)) / batch_size) - 1:
              generate_rotation_imgs(gen, self.batch_size, batch_z, epoch + 1)
          pbar.update(1)

        pbar.close()
          
        if (epoch+1) % save_step == 0 and model_dir is not None:
            ...
            dd['epoch'] = epoch + 1
            torch.save(dd, "%s/%i.pkl" % (model_dir, epoch+1))
  ...
  def train_on_batch(self, data):
      ...
      disc_fake_loss = nn.functional.binary_cross_entropy_with_logits(disc_logits_fake, torch.zeros_like(disc_logits_fake))
      disc_real_loss = torch.nn.functional.binary_cross_entropy_with_logits(disc_logits_real, torch.ones_like(disc_logits_real)) 
